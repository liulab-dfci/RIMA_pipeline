...
CoreAlg <- function(X, y, absolute, abs_method){
    ...
    q[which(q<0)]<-0
    if(absolute != TRUE || abs_method == 'sig.score') w <- (q/sum(q)) #relative space (returns fractions)
    if(absolute == TRUE && abs_method == 'no.sumto1') w <- q #absolute space (returns scores)
...
CIBERSORT <- function(sig_matrix, mixture_file, perm=0, QN=TRUE, absolute=FALSE, abs_method='sig.score'){
    ...
    if(absolute == TRUE && abs_method != 'no.sumto1' && abs_method != 'sig.score') stop("abs_method must be set to either 'sig.score' or 'no.sumto1'")
    
    #read in data
    X <- read.table(sig_matrix,header=TRUE,sep="\t",row.names=1,check.names=FALSE)
    Y <- read.table(mixture_file, header=TRUE, sep=",",check.names=F,stringsAsFactors = FALSE)
    ...
    if(absolute == TRUE) header <- c(header, paste('Absolute score (',abs_method,')',sep=""))
    ...
    while(itor <= mixtures){
        ...
        if(absolute == TRUE && abs_method == 'sig.score') {
        ...
        if(absolute == TRUE) out <- c(out, sum(w))
    ...
    if(absolute != TRUE){colnames(obj) <- c(colnames(X),"P-value","Correlation","RMSE")}
    else{colnames(obj) <- c(colnames(X),"P-value","Correlation","RMSE",paste('Absolute score (',abs_method,')',sep=""))}
    return (obj)
    #save results
    #write.table(obj, file = paste(opt$outdir,opt$prefix,'_cibersort.txt',sep = ""), quote = FALSE, sep="\t")
}
