...
option_list <- list(
  ...
  make_option(c("-q","--multiqc"), type = "character",
              help = "preprocess data for multiqc")
...
###test
# inputdir <- "~/Downloads/gejun_test_single/files/immunedeconv/"
# outdir <- "~/Downloads/gejun_test_single/"
# col <- "Tissue"
# meta <- read.table("~/Downloads/gejun_test_single/metasheet.csv",sep = ",", header = TRUE, row.names = 1)
...
gr <- lapply(phenos, function(p){
  if (length(which(meta[,col] == p)) == 1) {
    ss <- rownames(meta)[which(meta[,col] == p)]
  } else {
...
#remove the cell type can't be found in all of samples
tmp_merge <- merge.df
tmp_merge <- cbind(tmp_merge, rowSums(tmp_merge))
colnames(tmp_merge)[ncol(tmp_merge)] <- "sum"
tmp_merge <- as.data.frame(tmp_merge)
tmp_merge <- subset(tmp_merge, sum > 0)
tmp_merge <- tmp_merge[-ncol(tmp_merge)]

merge.df <- as.matrix(tmp_merge)
...
png(paste(outdir,"/ImmuneDeconv_heatmpap.png",sep = ""), res = 450, width = 7000, height =Pheight)
...
#  if (f == "timer_immune_multiqc.txt") {
#    min_max <- function(x) {
#      return((x - min(x)) / (max(x) - min(x)))
#    }
#    tmp <- apply(tmp, MARGIN = 1, min_max)
#    tmp <- t(tmp)
