...
#####read in data
#data <- read.table(opt$deseq2_mat, sep = "\t", header = TRUE)
...
for(n in 1:length(Condition)) {
  id <- paste0(Condition[n], "_", Treatment[n], "_vs_", Control[n])
  tmp_mat <- paste0("analysis/deseq2/",id, "_DESeq2.txt")
  terms <- append(terms, id)
  datalist <- append(datalist, tmp_mat)

}
####read in data
for(i in 1:length(datalist)) {
  data <- read.table(datalist[[i]], sep = "\t", header = TRUE)
  ...
  ###volcano plot
  png(paste0("analysis/deseq2/", terms[[i]], "_volcano_plot.png"), res = 300, width = 1900, height = 1000)
