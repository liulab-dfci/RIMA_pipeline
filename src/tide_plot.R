...
###test
# input <- "~/Documents/pipeline_test/RIMA_V1/tpm_convertID_batch_tide_score.txt"
# expr <- "~/Documents/pipeline_test/RIMA_V1/tpm_convertID_batch_Entrez_normalize_control.txt"
...
png(file=paste(outdir,"tide_score.png",sep = ""), res = 300, width = Pwidth, height = 1400)
...
if (cc %in% names(TIDE.res)){
  ...
  ha1 = HeatmapAnnotation(Zhao_Data = c(rep("User_Data",ncol(user.mat))), 
                          col = list(Zhao_Data = c("User_Data" = "pink")),
			  show_annotation_name = FALSE, size = 0.1)
  ha2 = HeatmapAnnotation(TCGA_GBM = c(rep("TCGA_Data",ncol(tcga.mat))), 
                          col = list(TCGA_GBM = c("TCGA_Data" = "royalblue")),
  ...
  ht_list = Heatmap(user.mat.norm,  name = "Score",col = col_fun,
    ...
    Heatmap(tcga.mat.norm,  name = "Score",col = col_fun,
	    ...
	    heatmap_legend_param = list( legend_direction="horizontal", legend_width=unit(3,"mm")),
            row_names_gp = gpar(fontsize = 6))
  png(file=paste(outdir,"tide_user_tcga.png",sep = ""), res = 300, width = 2000, height = 700)
