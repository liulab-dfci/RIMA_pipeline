...
option_list = list(
  make_option(c("-f", "--clonality"), type="character", default=NULL,
...
Control <- opt$control
outdir <- opt$outdir
files <- unlist(strsplit(opt$clonality,","))
...
clonality <- merge_process(filelist.samples)
write.table(clonality, paste(outdir,Condition,"_",Treatment,"_vs_",Control,"_TRUST4_BCR_clonality.txt",sep=''),sep = '\t',quote=FALSE,row.names = FALSE)
