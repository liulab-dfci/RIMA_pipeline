...
if ("Pre" %in% meta[["Timing"]]) {
  ...
  input.tide.post.f <- input.tide.post-rowMeans(input.tide.post)
  ...
  write.table(data.frame("gene_id"=rownames(input.tide.post.f),input.tide.post.f),paste(opt$outdir,"tide/tideinput.others.txt",sep=""),quote=F,sep="\t",row.names=FALSE)
}
